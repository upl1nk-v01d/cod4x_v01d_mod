...
finalkillcam( attacker, attackerNum, deathtime, victim)
{
	...
	self.spectatorclient = victimNum;
...
endGame( winner, endReasonText )
{
    ...
    if ( (level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd )
    {
        ...
        if(level.players.size > 0 && level.gametype == "sab" && !maps\mp\gametypes\_globallogic::hitScoreLimit())
        {
            level.killcam_style = 1;
            if(isDefined(winner)) { thread startFK( winner ); }
        }
    ...
    if(level.gametype == "sab" && maps\mp\gametypes\_globallogic::hitScoreLimit() && level.players.size > 0)
    {
        level.killcam_style = 0;
        thread startFK( winner );
    }
