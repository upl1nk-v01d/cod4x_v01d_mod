...
linkWaypoints()
{
	...
	for ( ;; )
	{
		while(!self attackButtonPressed() || isDefined(self.command))
			wait 0.05;

		self.command = true;

		self iprintln("Link Wpt");
...
unlinkWaypoints()
{
	...
	for(;;)
	{
		while(!self AdsButtonPressed() || isDefined(self.command))
			wait 0.05;
		
		self.command = true;
		
		self iprintln("UnLink Wpt");
...
deleteOneWaypoint()
{
	...
	for(;;)
	{
		while(!self UseButtonPressed() || isDefined(self.command)) //SecondaryOffhandButtonPressed()
			wait 0.05;
		
		self.command = true;
		
		self iprintln("DeleteWpt");
